(function (global, factory) {
	...
	function initConfig(config) {
	    ...
	    if (!config.environment) {
	        throw new Error('PaywallJsSdk.Init() requires environment parameter');
	    }
	    if (!config.token || config.token.trim() === '') {
	        throw new Error('PaywallJsSdk.Init() requires token');
	    }
	...
	function getConfig() {
	    if (!currentConfig) {
	        throw new Error('PaywallJsSdk is not initialized. Call PaywallJsSdk.Init() first.');
	    }
	...
	const SDK_MESSAGES = {
	    // SDK Initialization
	    SDK_NOT_INITIALIZED: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before using this function.',
	...
	async function addCard(params) {
	    if (!isInitialized()) {
	        return createFailedResponse('SDK', 'PaywallJsSdk is not initialized. Call PaywallJsSdk.Init() first.', 'SDK_NOT_INITIALIZED');
	    }
	...
	const PaywallJsSdk = {
	     ...
	     *
	     * **LIFECYCLE:**
	     * - SADECE config validation + state set eder
	     * - ASLA network isteği atmaz
	     * - SDK lifecycle state: UNINITIALIZED → INITIALIZED
	     *
	     * **TOKEN MANTIĞI:**
	     * - Token config.token içinde alınır
	     * - SDK token üretmez, refresh etmez
	     * - Merchant secret key alınmaz
	     *
	     * **ENVIRONMENT YÖNETİMİ:**
	     * - Environment (dev, test, prod) otomatik resolve edilir
	     * - Paywall Base URLs ve Masterpass SDK URLs otomatik seçilir
	     * - Merchant bu URL'leri manuel veremez
	     *
	     * @param config - SDK konfigürasyonu (environment ve token)
	     * @throws Error - Eksik veya geçersiz config parametreleri varsa
	     *
	     * @example
	     * ```typescript
	     ...
	     * // Network tab boş olmalı
	     * ```
	     */
	    async Init(config) {
	        // ⚠️ KRİTİK: ASLA throw ETME, her zaman response döndür
	        try {
	            // Token kontrolü - boş mu?
	            if (!config.token || config.token.trim() === '') {
	                return {
	                    success: false,
	                    status: 'FAILED',
	            ...
	            // Token kontrolü - GUID formatında mı?
	            // GUID formatı: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (36 karakter, 8-4-4-4-12 formatında)
	            const guidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
	            if (!guidPattern.test(config.token.trim())) {
	                return {
	                    success: false,
	                    status: 'FAILED',
	                    source: 'SDK',
	                    message: 'Token must be in GUID format (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx). Please provide a valid GUID token.',
	                    data: {
	                        environment: config.environment,
	                        sdkInitialized: false,
	                        providerMeta: {
	                            responseCode: 'INVALID_TOKEN_FORMAT',
	                        },
	                    },
	                };
	            }
	            // Convert to PaywallJsSdkConfig (with defaults)
	            const fullConfig = {
	                environment: config.environment,
	    ...
	    payment: {
	         ...
	         * - Bu fonksiyon çağrılmadan önce mutlaka `PaywallJsSdk.Init(config)` çağrılmış olmalıdır.
	         ...
	         * await PaywallJsSdk.Init(config);
	         *
	    ...
	    providers: {
	         ...
	         *
	         * **LIFECYCLE:**
	         * - SDK core init edilmiş olmalıdır (PaywallJsSdk.Init() çağrılmış olmalı)
	            ...
	            init: async () => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before provider init.',
	                        data: {
	                            masterpassSdkInitialized: false,
	                        },
	                    };
	            ...
	            addCard: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before addCard.',
	                        providerMeta: {
	                            provider: 'SDK',
	                            httpStatus: 0,
	            ...
	            linkMerchant: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before linkMerchant.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            unlinkMerchant: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before unlinkMerchant.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            verifyOtp: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before verifyOtp.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            resendOtp: async () => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before resendOtp.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            getCardList: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before getCardList.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            merchantUnlink: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before merchantUnlink.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
	            ...
	            deleteCard: async (params) => {
	                ...
	                if (!isSdkInitialized()) {
	                    return {
	                        ...
	                        status: 'FAILED',
	                        source: 'SDK',
	                        message: 'SDK core must be initialized first. Call PaywallJsSdk.Init() before deleteCard.',
	                        providerMeta: {
	                            responseCode: 'SDK_NOT_INITIALIZED',
	                        },
	                    };
